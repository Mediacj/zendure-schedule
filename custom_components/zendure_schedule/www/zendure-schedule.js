/*
 * Zendure Schedule — tiny Lovelace loader/stub.
 *
 * Home Assistant waits only ~2s for custom elements. This small file registers
 * `zendure-schedule` immediately, then dynamically loads the full card.
 * Register THIS file as the Lovelace resource / extra_module_url — not the card.
 */
(() => {
  ...
  const CARD_URL = `/zendure_schedule/zendure-schedule-card.js?v=${VERSION}`;
  ...
  // Load the full card once; ES modules are cached per URL.
  if (!window.__zendureScheduleCardLoading) {
    ...
    import(CARD_URL)
      ...
      .catch((err) => {
        console.error("Zendure Schedule: laden van card-module mislukt", CARD_URL, err);
        walkShadow(document, (root) => {
          root.querySelectorAll?.(TAG).forEach((el) => {
            el.innerHTML =
